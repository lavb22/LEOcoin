...
public:
    ...
    int64_t GetLEOcoinBalance() const;
    ...
    bool SendLEOToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToLEO(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetLEOcoinDebit(const CTxIn& txin) const;
    int64_t GetLEOcoinCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetLEOcoinCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nLEO, int64_t& nLEOcoin) const
    {
        nLEO = 0;
        nLEOcoin = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nLEOcoin += GetLEOcoinCredit(txout);
            } else
                nLEO += GetCredit(txout);
            if (!MoneyRange(nLEO)
                || !MoneyRange(nLEOcoin))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableLEOcoinCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableLEOcoinCreditCached;
    
    mutable int64_t nCredLEOCached;
    mutable int64_t nCredLEOcoinCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableLEOcoinCreditCached = false;
        
        nCredLEOCached = 0;
        nCredLEOcoinCached = 0;
        ...
        nAvailableLEOcoinCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredLEO, int64_t& nCredLEOcoin, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredLEO = nCredLEOcoin = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredLEOCached = 0;
            nCredLEOcoinCached = 0;
            pwallet->GetCredit(*this, nCredLEOCached, nCredLEOcoinCached);
            fCreditSplitCached = true;
        };
        
        nCredLEO = nCredLEOCached;
        nCredLEOcoin = nCredLEOcoinCached;
        return true;
    }
    ...
    int64_t GetAvailableLEOcoinCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableLEOcoinCreditCached)
            return nAvailableLEOcoinCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableLEOcoinCredit() : value out of range");
            };
        };
        
        nAvailableLEOcoinCreditCached = nCredit;
        fAvailableLEOcoinCreditCached = true;
        return nCredit;
    };
